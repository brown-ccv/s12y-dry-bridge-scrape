...
def process_inverter_data(timestamp, kw):
    kwh = kw * 0.25
    mmbtu = kwh * 0.003412
    mtco2e = (kwh * 0.0002369153) + (kwh * 0.0000000371952 * 28) + (kwh * 0.0000000049896 * 265)
...
def scrape_range(browser, start, end, connection):
    ...
    for day in daterange(start, end):
        ...
        if not download_success:
            print(f"All {MAX_RETRIES} attempts failed to download CSV for {day.strftime(DATE_FORMAT)}")
